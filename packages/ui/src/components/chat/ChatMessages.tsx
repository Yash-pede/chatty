...
export const ChatMessages = ({ messages, userData }: { messages: ChatMessage[], userData: ChatUser }) => {
            ...
            {messages.map((message: ChatMessage) => {
                ...
                return (
                    ...
                    >
                        {/* TEXT MESSAGE */}
                        {message.type === "text" && (
                            <Item className="leading-relaxed p-0">{(message.content as MessageContentType)?.text}</Item>
                        )}
                        ...
                        <ItemGroup className="flex-row justify-end gap-1" >
                            <Item className="p-0 text-xs"> {message.createdAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</Item>
